...
from nrobo import console, terminal, STYLE, NroboConst, __version__
...
def update_available() -> bool:
    ...
    return Version(get_host_version()) < Version(get_pypi_index(NroboConst.NROBO))
...
def confirm_update() -> None:
    ...
    if (
        Version.present_is_a_patch_release(pypi_version.version, host_version.version)
        or Version.present_is_a_minor_release(
            pypi_version.version, host_version.version
        )
        or Version.present_is_a_major_release(
            pypi_version.version, host_version.version
        )
    ):
        ...
        if Version.present_is_a_major_release(
            ...
            terminal(
                [
                    ...
                    "install",
                    "--upgrade",
                    f"nrobo=={(host_version + 1).version}",
                    "--require-virtualenv",
                ],
                debug=False,
            )
        ...
        if (
            detect.production_machine()
            and not detect.developer_machine()
